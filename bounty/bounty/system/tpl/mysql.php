...
class Mysql {
	...
	var $cache_data_dir					= 'cache/cache/';
	var $mysql_query_log_path			= 'cache/caches/';
	var $root_path						= '';
	 ...
	 * @param number $pconnect        	
	 * @param number $quiet        	
	 */
	function __construct($dbhost, $dbuser, $dbpw, $dbname = '', $prefix='', $charset = 'utf8', $pconnect = 0, $quiet = 0) {
		$this->db_name = $dbname;
		$this->prefix = $prefix;
		if (defined ( 'ROOTPATH' ) && ! $this->root_path) {
	...
	function query($sql, $type = '') {
		...
		if (defined ( 'DEBUG_MODE' ) && (DEBUG_MODE & 8) == 8) {
			$logfilename = $this->root_path . $this->mysql_query_log_path . 'mysql_query_' . date ( 'Y_m_d' ) . '.log';
			$str = $sql . "\n\n";
	...
	function ErrorMsg($message = '', $sql = '') {
		if ($message) {
			echo "<b>system-defined info</b>: $message\n\n<br /><br />";
		} else {
	...
	function table($table) {
		return "`{$this->db_name}`.`{$this->prefix}{$table}`";
	}
}
