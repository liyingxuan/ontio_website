...
class Articlem extends CI_Model{
     ...
     * @author  吴龙
     */ 
    public function getArchives($where,$cols='*',$curpage=1,$rp=40,$sortname='a.pubdate',$sortorder='desc'){
        if(is_string($where)&&is_string($cols))
        {
            $this->db->select($cols)->from('archives a')->join('arctype t','t.id=a.typeid','left')->join('language g','g.lgid=a.lgid','left')->join('admin m','m.admin_id=a.dutyadmin','left')->join('channeltype c','c.id=t.channeltype','left')->where($where);
            $db=clone($this->db);
     ...
     * @param  array $where1 主表添加的数据
     * @param  array $where2 副表添加的数据
     * @return 1，参数异常：返回false数组 2，正常：返回true数组
     * @author  吴龙
     */ 
    public function addArticle($tables,$data1,$data2){
        if(is_array($data1)&&is_array($data2))
        {
        	// $this->db->trans_begin();
           	$this->db->insert('archives', $data1);
            ...
            if(isset($tables[1])&&!empty($tables[1])){//存在副表
            	$data2['aid'] = $id;
            	// var_dump($data2);die;
           		$this->db->insert('addonarticle', $data2);
           		$error = $this->db->error();
	            if(!empty($error['code'])){
	            	$this->db->trans_rollback();
            ...
            return array('state' => true,'msg'=>'添加成功','data'=>'');
        }else{
     ...
     * @author  吴龙
     */ 
    public function delArticle($tables,$where1,$where2){
     ...
     * @author  吴龙
     */ 
    public function editArticle($tables,$data1,$data2,$where1,$where2){
     ...
     * 获取栏目树
     */
    public function getColumnList($pid = 0, $level = 0)
    {
        // $data = $this->db->select('reid,id,channeltype,typename,ispart')->order_by('sortrank')->where(array('reid' => $pid))->get('arctype')->result_array();
        $data = $this->db->select('a.reid,a.id,a.channeltype,a.typename,a.ispart,c.nid')->from('arctype a')->join('channeltype c','c.id=a.channeltype','left')->where(array('a.reid' => $pid))->get()->result_array();
        $level++;
        $error = $this->db->error();
        if (!empty($error['code'])) {
            return array('state' => false, 'msg' => "查询错误!".$error['code'].$error['message'], 'data' => '');
     ...
     * @return 1，参数异常：返回false数组 2，正常：返回true数组
     */
    public function editorArctype($data, $where)
    {
     ...
     * @return 1，参数异常：返回false数组 2，正常：返回true数组
     */
    public function addArctype($data)
    {
        if (is_array($data)) {
